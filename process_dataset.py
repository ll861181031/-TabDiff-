...
def preprocess_avoid():
    """
    Preprocesses the avoid dataset by ensuring weather columns are mutually exclusive
    and filling missing values
    """
    print("Preprocessing avoid dataset...")
    with open(f'{INFO_PATH}/avoid.json', 'r') as f:
        info = json.load(f)
    ...
    # Load train data
    train_path = info.get('data_path', 'data/avoid_train.csv')
    if os.path.exists(train_path):
        train_df = pd.read_csv(train_path)
        print(f"Loaded train data: {train_df.shape}")

        # Make weather columns mutually exclusive
        for idx, row in train_df.iterrows():
            marked_weather = [col for col in weather_columns if col in train_df.columns and str(row[col]).strip().upper() == 'Y']
            if len(marked_weather) > 1:
                first_weather = marked_weather[0]
                for col in weather_columns:
                    if col in train_df.columns:
                        train_df.at[idx, col] = 'Y' if col == first_weather else ' '
            elif len(marked_weather) == 1:
                for col in weather_columns:
                    if col in train_df.columns and col not in marked_weather:
                        train_df.at[idx, col] = ' '
            else:
                for col in weather_columns:
                    if col in train_df.columns:
        ...
        num_columns = ['Posted Speed Limit (MPH)', 'SV Precrash Speed (MPH)', 'Model Year', 'Mileage']
        for col in num_columns:
            if col in train_df.columns:
                ...
                if train_df[col].isna().all():
                    ...
                    print(f"  {col}: All NaN, filled with 0")
                else:
                    median_val = train_df[col].median()
                    train_df[col] = train_df[col].fillna(median_val)
                    print(f"  {col}: Filled {train_df[col].isna().sum()} NaN with median {median_val}")

        # Fill empty strings in categorical columns with 'Unknown'
        for col in train_df.columns:
            if col not in num_columns:
                train_df[col] = train_df[col].fillna('Unknown')
                train_df[col] = train_df[col].replace('', 'Unknown')
                train_df[col] = train_df[col].replace(' ', ' ')  # Keep single space as is

        # Save processed train data
        output_path = 'data/avoid/train.csv'
        os.makedirs('data/avoid', exist_ok=True)
        train_df.to_csv(output_path, index=False)
        print(f"Saved processed train data to {output_path}: {train_df.shape}")

    # Load test data
    test_path = info.get('test_path', 'data/avoid_test.csv')
    if os.path.exists(test_path):
        ...
        for idx, row in test_df.iterrows():
            marked_weather = [col for col in weather_columns if col in test_df.columns and str(row[col]).strip().upper() == 'Y']
            if len(marked_weather) > 1:
                first_weather = marked_weather[0]
                for col in weather_columns:
                    if col in test_df.columns:
                        test_df.at[idx, col] = 'Y' if col == first_weather else ' '
            elif len(marked_weather) == 1:
                for col in weather_columns:
                    if col in test_df.columns and col not in marked_weather:
                        test_df.at[idx, col] = ' '
            else:
                for col in weather_columns:
                    if col in test_df.columns:
        ...
        for col in num_columns:
            if col in test_df.columns:
                ...
                # Fill NaN with median or 0
                if test_df[col].isna().all():
                    test_df[col] = test_df[col].fillna(0.0)
                    print(f"  {col}: All NaN, filled with 0")
                else:
                    median_val = test_df[col].median()
                    test_df[col] = test_df[col].fillna(median_val)
                    print(f"  {col}: Filled {test_df[col].isna().sum()} NaN with median {median_val}")

        # Fill empty strings in categorical columns
        for col in test_df.columns:
            if col not in num_columns:
                test_df[col] = test_df[col].fillna('Unknown')
                test_df[col] = test_df[col].replace('', 'Unknown')
                test_df[col] = test_df[col].replace(' ', ' ')  # Keep single space as is

        # Save processed test data
        output_path = 'data/avoid/test.csv'
        test_df.to_csv(output_path, index=False)
        print(f"Saved processed test data to {output_path}: {test_df.shape}")

    # Update info.json to point to processed files
    info['data_path'] = 'data/avoid/train.csv'
    info['test_path'] = 'data/avoid/test.csv'
    with open(f'{INFO_PATH}/avoid.json', 'w') as f:
        json.dump(info, f, indent=4)
    print("Updated avoid.json configuration")
...
def process_data(name):
    ...
    for i, col_idx in enumerate(num_col_idx):
        ...
        try:
            ...
            is_int = (col_data_numeric.dropna() % 1 == 0).all()
            if is_int:
                int_columns.append(col)
                int_col_idx.append(name_idx_mapping[col])
                int_col_idx_wrt_num.append(i)
        except Exception as e:
            print(f"Warning: Could not check if column '{col}' contains integers: {e}")
            continue
    ...
    for col_idx in num_col_idx:
        ...
        col_data = pd.to_numeric(train_df[col_idx], errors='coerce')
